...
package com.android.systemui.infinity.batterybar;
...
public class BatteryBar extends RelativeLayout implements Animatable, TunerService.Tunable {
    ...
    private static int getBlendColorForPercent(int fullColor, int emptyColor, boolean reversed,
        ...
        Color.colorToHSV(fullColor, fullHSV);
        Color.colorToHSV(emptyColor, emptyHSV);

        int fullAlpha = Color.alpha(fullColor);
        int emptyAlpha = Color.alpha(emptyColor);
        float blendFactor = percentage / 100f;

        if (Math.abs(fullHSV[0] - emptyHSV[0]) > 180) {
            if (fullHSV[0] > emptyHSV[0]) {
                emptyHSV[0] += 360f;
            } else {
                fullHSV[0] += 360f;
            }
        }
        float blendedHue = reversed
                ? fullHSV[0] - (fullHSV[0] - emptyHSV[0]) * blendFactor
                : emptyHSV[0] + (fullHSV[0] - emptyHSV[0]) * blendFactor;

        blendedHue = (blendedHue + 360f) % 360f;

        float blendedSaturation = Math.max(0f, Math.min(1f, emptyHSV[1] + (fullHSV[1] - emptyHSV[1]) * blendFactor));
        float blendedValue = Math.max(0f, Math.min(1f, emptyHSV[2] + (fullHSV[2] - emptyHSV[2]) * blendFactor));
        int blendedAlpha = Math.max(0, Math.min(255, (int) (emptyAlpha + (fullAlpha - emptyAlpha) * blendFactor)));
        return Color.HSVToColor(blendedAlpha, new float[]{blendedHue, blendedSaturation, blendedValue});
    }
